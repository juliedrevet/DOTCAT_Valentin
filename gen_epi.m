...
% !! differenciate if nepi = 2 !!
if nepi == 2
    found = false;
    xs1 = find(rand < cs,1,'first');
    while (abs(2*xs1-nepi*davg) >= round(davg/3))
        xs1 = find(rand < cs,1,'first');
    end
    if (abs(2*xs1-nepi*davg) < round(davg/3))
        xs = [xs1 nepi*davg-xs1];
        xs = xs(randperm(nepi)); % repermute
        fm = @(p,x)-sum(log(p)+log(1-p)*x)+nepi*log(sum(p*(1-p).^(dmin:dmax)));
        pr = fminbnd(@(p)fm(p,xs),0.001,0.999);
        ys = unpack(xs);
        found = true;
    end
else
    ...
    for iseq = 1:nseq
        ...
        for iper = 1:nper
            ...
            if  (mod(nepi,2) == 0) && ... % even number of episodes
                    ...
                    ((abs(diff(hist(ys(1:ns/2),[1,2]))) < round(davg/3) && ...
                    ...% control the number of 1 and 2 in the 2nd half: difference < round(davg/2)
                    abs(diff(hist(ys(ns/2+1:end),[1,2]))) < round(davg/3))))
                found = true;
                break
